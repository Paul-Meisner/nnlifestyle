...
let search = $state("");
let allTags = $derived.by(() => Array.from(new Set(sortedPosts.flatMap(post => post.data.tags))).sort());
let allCategories = $derived.by(() => Array.from(new Set(sortedPosts.map(post => post.data.category).filter(Boolean))).sort());
...
let groups = $derived.by(() => {
    ...
    if (search.trim().length > 0) {
        ...
        filteredPosts = filteredPosts.filter(post =>
            ...
            (post.data.description && post.data.description.toLowerCase().includes(q))
        );
    }
...
<div class="mb-6 flex flex-col md:flex-row md:items-end gap-4">
    ...
    />
    <div class="flex flex-col w-full md:w-1/4 min-w-[220px]">
        <select
            id="category-select"
            class="select select-bordered w-full min-w-[220px]"
            bind:value={categories[0]}
            on:change={(e) => {
