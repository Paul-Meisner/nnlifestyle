...
export function getTagUrl(tag: string): string {
    if (!tag) return url("/articles/");
    return url(`/articles/?tag=${encodeURIComponent(tag.trim())}`);
}
...
export function getCategoryUrl(category: string | null): string {
    if (
        ...
        return url("/articles/?uncategorized=true");
    return url(`/articles/?category=${encodeURIComponent(category.trim())}`);
}
